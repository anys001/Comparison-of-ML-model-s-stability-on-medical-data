...
TARGET_NAME_SRC = 'diagnosis'
TARGET_NAME = 'Target'
# reading datas
datam = pd.read_csv("./Data/data2.csv", delimiter=';')
data = datam.drop(['id','radius_mean','area_mean', 'perimeter_mean', 'concavity_mean','concave points_mean', 'compactness_mean',
                   'radius_se','area_se','perimeter_se', 'compactness_worst','area_worst','radius_worst','perimeter_worst',
                   'concavity_worst','concave points_worst'], axis=1)
data[TARGET_NAME] = data[TARGET_NAME_SRC].map({'B': 0, 'M': 1})

LABELS = { 0: "Benign", 1: "Malignant"}
FEATURES = data.drop(columns=['diagnosis', 'Target']).columns.tolist()
...
BestModel_rg, _ = modelfunc(data = data_prepared, modelnm = MODEL_NAME) #main

# Main experiment part

def stab_alldf(data, step, sample_ratio, BestModel, model_name, optimize='N', restore_state_path=None, download_state=False):
    ...
    while percent_removed < 91:
        ...
        sampled_data = data.sample(n=n_samples, random_state=42) if n_samples < len(data) else data

        accuracy = maintr_new(data=sampled_data, sample=sample_ratio, best_model=BestModel, optfl=optimize, modelnm=model_name, prepare_data=prepare_datasets, random_state=42)
