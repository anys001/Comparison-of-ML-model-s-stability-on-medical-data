...
dis1 = datam[datam[TARGET_NAME_SRC].isin([2.0])]
dis2 = dis0.sample(n=37000, random_state=42)
data = pd.concat([dis1, dis2])

data[TARGET_NAME] = data[TARGET_NAME_SRC].map({0.0: 0, 2.0: 1})
...
def modelfunc(data, modelnm, random_state=42, n_jobs1 = n_jobsf):
  ...
  if modelnm == "Regression":
   ...
   # Training using GridSearchCV
    ...
    best_model = CatBoostClassifier(eval_metric='AUC:hints=skip_train~false', random_seed=random_state, loss_function='CrossEntropy', verbose=10)
    ...
    param_grid = {
        ...
        'l2_leaf_reg': [0.5, 2, 4, 6, 8],
        #CrossEntropy helps to close outlier
    }
...
BestModel_rg, _ = modelfunc(data = data_prepared, modelnm = "Bagging") #main
